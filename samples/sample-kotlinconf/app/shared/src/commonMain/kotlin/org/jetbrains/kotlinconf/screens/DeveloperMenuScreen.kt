...
@NavDestination(route = org.jetbrains.kotlinconf.navigation.DeveloperMenuScreen::class)
@Composable
fun DeveloperMenuScreen(
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        AnimatedVisibility(showWarning, modifier = Modifier.fillMaxSize()) {
            ...
            DeveloperWarningOverlay(
                enabled = enabled,
                onProceed = { showWarning = false },
                onBack = onBack,
            )
        }
    }
...
@Composable
private fun DeveloperWarningOverlay(
    enabled: Boolean,
    onProceed: () -> Unit,
    onBack: () -> Unit,
...
@NavPreview(route = org.jetbrains.kotlinconf.navigation.DeveloperMenuScreen::class, primary = true)
@Preview
@Composable
private fun DeveloperMenuScreenNavPreview() {
    // The screen opens on a full-screen warning overlay (showWarning = true) before any
    // settings are shown. That overlay is fully stateless and reads no DI graph, so we
    // preview it directly. Rendering the whole DeveloperMenuScreen would require a live
    // AppGraph (LocalAppGraph), which the Layoutlib render does not provide.
    KotlinConfTheme {
        DeveloperWarningOverlay(
            enabled = true,
            onProceed = {},
