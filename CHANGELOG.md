...
- **`navLint`**: a health check on the merged graph, where the `.nav` baseline only reviews change. Reports screens with no path from the start destination, a missing or duplicated start, and routes no `@NavDestination` binds. Warns by default (`navgraph { failOnNavLint }` to gate CI). It reads the aggregated graph, so it is not wired into `check` unless you ask (`navgraph { lintOnCheck }`) — a single module's own manifest cannot tell a route another module owns from a genuinely unbound one. Silence a rule with `navLintDisabledRules` or a route with `navLintIgnoredRoutes`. Note `unreachable` is a stricter question than the exports' "Unconnected screens" section, which is a decluttering device that ignores edge direction.
